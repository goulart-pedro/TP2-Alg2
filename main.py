...
class KCenter:
    ...
    def _compute_distance_matrix(self, metric: str):
        """Calcula matriz de distância (Versão vetorizada/rápida)."""
        if metric == 'euclidean':
            ...
            self.distance_matrix = calculate_minkowski_matrix(self.data_tp2, self.data_tp2, p=3) # Exemplo p=3
    ...
    def inertia(self, center_coordinates, labels):
        """Calcula a inércia (Within-Cluster Sum of Squares)."""
        if labels is None: return 0.0
        total_inertia = 0.0
        # Otimização com numpy ao invés de loop python puro
...
def run_original_demo():
    ...
    print("Salvando imagem em 'image.svg'...")
...
if __name__ == "__main__":
    while True:
        print("\n=== TP2 - K-Center ===")
        print("1. Demo Visual (Seu código original)")
        print("2. Gerar Relatório Completo (Experimentos)")
        print("0. Sair")
